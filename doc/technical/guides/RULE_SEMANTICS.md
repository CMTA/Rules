...
12b. **`RuleWhitelistWrapper` children must be ALLOW-lists, and nothing enforces it.** `IAddressList` carries *membership*, not polarity: the wrapper ORs its children's `areAddressesListed` answers and reads `true` as **eligible**. A `RuleBlacklist` implements the same interface, advertises the same interface id and passes every check `addRule` performs — but its set means the opposite, so adding one makes its blacklisted addresses whitelisted and `isVerified` returns `true` for them. An ERC-165 guard would not catch it; distinguishing polarity would need a separate marker interface. Until one exists this is configuration discipline for the `RULES_MANAGEMENT_ROLE` holder. Unlike an empty wrapper, which fails **closed**, this fails **open** and silently. Nethermind AuditAgent `NM-20`. What the wrapper **does** enforce is ERC-165: a child must advertise `IAddressListBatchQuery` (`0x20e8e17a`, the single `areAddressesListed(address[])` selector — not the full `IAddressList` id, since that is the only function the wrapper calls), else `addRule` reverts with `RuleWhitelistWrapper_ChildIsNotAnAddressList` (`NM-18`, audit `F-5`). That also refuses a nested wrapper, which does not implement the function.
