...
- `RuleWhitelistWrapper` **does** ERC-165-check its child rules, via a `_checkRule` override mirroring `RuleEngineBase` (NM-18, F-5). It requires `IADDRESS_LIST_BATCH_QUERY_INTERFACE_ID` (`0x20e8e17a`, the single `areAddressesListed(address[])` selector), **not** the full `IADDRESS_LIST_INTERFACE_ID`: that one function is all the wrapper calls, so demanding the other seven — four of them writes — would reject a valid read-only child. `IAddressListBatchQuery` was split out of `IAddressList` for this; the flattened selector set is unchanged, so `0x5d10e182` keeps its value, and the sub-interface id is safe as a literal because it inherits nothing. **The guard cannot check polarity, and children must still be ALLOW-lists**: `IAddressList` carries membership only, so a `RuleBlacklist` advertises the same ids, passes the guard, and inverts the wrapper — its blacklisted addresses become whitelisted and `isVerified` returns `true` for them (NM-20). That stays configuration discipline; pinned by `test_WW2_GuardCannotRejectAnInvertedPolarityChild_CurrentBehaviour`.
