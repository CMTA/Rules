...
contract CMTATIntegration is Test, HelperContract {
    ...
    function testCannotTransferWithoutAddressWhitelisted() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RuleWhitelist_InvalidTransfer.selector, address(ruleWhitelist), ADDRESS1, ADDRESS2, 21, CODE_ADDRESS_FROM_NOT_WHITELISTED
            )
        );
        // Act
    ...
    function testCannotTransferWithoutFromAddressWhitelisted() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RuleWhitelist_InvalidTransfer.selector, address(ruleWhitelist),ADDRESS1, ADDRESS2, amount, CODE_ADDRESS_FROM_NOT_WHITELISTED
            )
        );
        // Act
    ...
    function testCannotTransferWithoutToAddressWhitelisted() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RuleWhitelist_InvalidTransfer.selector, address(ruleWhitelist), ADDRESS1, ADDRESS2, amount, CODE_ADDRESS_TO_NOT_WHITELISTED
            )
        );
        // Act
        CMTAT_CONTRACT.transfer(ADDRESS2, amount);
