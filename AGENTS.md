...
- `RuleWhitelistWrapper` does not ERC-165-check its child rules (unlike `RuleEngineBase._checkRule`); a non-`IAddressList` child bricks the scan. **And children must be ALLOW-lists**: `IAddressList` carries membership, not polarity, so the wrapper reads any child's `true` as *eligible*. A `RuleBlacklist` satisfies the interface and passes `addRule`, but its set means the opposite — its blacklisted addresses become whitelisted and `isVerified` returns `true` for them (NM-20). An ERC-165 guard would not catch that, since a blacklist advertises the same interface id; polarity is configuration discipline, not a contract check.
